...
export default class MoviesDAO {
  ...
  static async getMoviesByCountry(countries) {
    ...
    */
    ...
    try {
      ...
      // wire.
      //)
      let results = await movies.find({countries: {$in: countries}}, {projection: {title: 1}}).toArray();
  ...
  static genreSearchQuery(genre) {
    ...
    */
    ...
    //const result = await movies.find({genre: {$in: searchGenre}}).toArray();
  ...
  static async getMovieByID(id) {
    try {
      ...
      */
      ...
      var errorMsg = `${e}`;
      if(errorMsg && errorMsg.indexOf('Argument passed in must be a single String of 12 bytes or a string of 24 hex characters') > -1) {
        return null;
      } else {
        throw e
      }
